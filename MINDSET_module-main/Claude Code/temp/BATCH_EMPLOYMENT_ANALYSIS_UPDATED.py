"""
BATCH EMPLOYMENT ANALYSIS - ALL 469 STRATEGY FILES (UPDATED)
...
- Full sectoral distribution for each strategy-country combination
- Supply chain spillover analysis

OUTPUT FILES:
1. employment_by_sector_country_strategy.csv - Detailed sector-level results
2. employment_by_country_strategy.csv - Country-level summary with direct/indirect breakdown
3. employment_by_strategy.csv - Strategy-level summary
4. employment_summary_consolidated.xlsx - Final dataset with multiple analysis sheets
...
print("="*80)
print("BATCH EMPLOYMENT ANALYSIS - ALL STRATEGY FILES (WITH SPILLOVER TRACKING)")
print("="*80)
print()
...
for idx, scenario_file in enumerate(scenario_files, 1):
    ...
    try:
        ...
        # Calculate direct vs indirect employment
        # Direct: where dy_inv_exog > 0 (sectors receiving direct demand)
        # Indirect: all other jobs (supply chain effects)
        direct_demand_mask = dy_inv_exog > 0.01
        direct_jobs = dempl[direct_demand_mask].sum()
        indirect_jobs = dempl[~direct_demand_mask].sum()
        ...
        for country in MRIO_BASE.COU_ID:
            ...
            # Calculate direct vs indirect at country level
            country_direct_mask = direct_demand_mask[c_start:c_end]
            country_direct_jobs = dempl[c_start:c_end][country_direct_mask].sum()
            country_indirect_jobs = dempl[c_start:c_end][~country_direct_mask].sum()
            ...
            all_results_country.append({
                ...
                'jobs_total': country_jobs,
                'jobs_direct': country_direct_jobs,
                'jobs_indirect': country_indirect_jobs,
            ...
            # Sector-level detail (ALL sectors, even zeros, for proper distribution tracking)
            for s_idx, sector in enumerate(sector_list):
                ...
                # Save all sectors (including zeros) to show complete distribution
                all_results_detailed.append({
                    'strategy': scenario_name,
                    'main_country': main_country,
                    'country': country,
                    'sector': sector,
                    'demand_kUSD': sector_demand,
                    'output_kUSD': sector_output,
                    'jobs': sector_jobs,
                    'is_direct': sector_is_direct  # Flag for direct vs indirect
                })
        ...
        all_results_strategy.append({
            ...
            'jobs_direct': direct_jobs,
            'jobs_indirect': indirect_jobs,
...
if len(all_results_country) > 0:
    ...
    with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
        # Sheet 1: Country-level (pivot: strategies × countries) - TOTAL JOBS
        pivot_jobs_total = df_country.pivot_table(
            index='strategy',
            columns='country',
            values='jobs_total',
            aggfunc='sum',
        ...
        pivot_jobs_total['main_country'] = df_country.drop_duplicates('strategy').set_index('strategy')['main_country']
        pivot_jobs_total['TOTAL'] = pivot_jobs_total[MRIO_BASE.COU_ID].sum(axis=1)
        pivot_jobs_total.to_excel(writer, sheet_name='Jobs_Total_by_Country')

        # Sheet 2: Direct jobs by country
        pivot_jobs_direct = df_country.pivot_table(
            index='strategy',
            columns='country',
            values='jobs_direct',
            aggfunc='sum',
        ...
        pivot_jobs_direct['TOTAL'] = pivot_jobs_direct[MRIO_BASE.COU_ID].sum(axis=1)
        pivot_jobs_direct.to_excel(writer, sheet_name='Jobs_Direct_by_Country')

        # Sheet 3: Indirect jobs by country (spillovers)
        pivot_jobs_indirect = df_country.pivot_table(
            index='strategy',
            columns='country',
            values='jobs_indirect',
            aggfunc='sum',
            fill_value=0
        )
        pivot_jobs_indirect['TOTAL'] = pivot_jobs_indirect[MRIO_BASE.COU_ID].sum(axis=1)
        pivot_jobs_indirect.to_excel(writer, sheet_name='Jobs_Indirect_by_Country')

        # Sheet 4: Strategy summary
        df_strategy.to_excel(writer, sheet_name='Strategy_Summary', index=False)

        # Sheet 5: Investment amounts
        ...
        # Sheet 7: Sectoral distribution - Direct jobs by sector and country
        df_detailed_nonzero = df_detailed[df_detailed['jobs'] > 0.01]
        df_direct_sectors = df_detailed_nonzero[df_detailed_nonzero['is_direct'] == True]
        if len(df_direct_sectors) > 0:
            df_direct_sectors[['strategy', 'main_country', 'country', 'sector', 'jobs', 'output_kUSD']].to_excel(
                writer, sheet_name='Direct_Jobs_by_Sector', index=False
            )

        # Sheet 8: Sectoral distribution - Indirect jobs by sector and country
        df_indirect_sectors = df_detailed_nonzero[df_detailed_nonzero['is_direct'] == False]
        if len(df_indirect_sectors) > 0:
            df_indirect_sectors[['strategy', 'main_country', 'country', 'sector', 'jobs', 'output_kUSD']].to_excel(
                writer, sheet_name='Indirect_Jobs_by_Sector', index=False
            )

    print(f"[OK] Saved consolidated Excel: {output_file}")
    print(f"     Sheets: 8 sheets including sectoral distributions and direct/indirect breakdowns")
...
if len(all_results_strategy) > 0:
    ...
    print(f"  Mean Direct:   {df_strategy['jobs_direct'].mean():.1f} ({df_strategy['jobs_direct'].mean() / df_strategy['total_jobs'].mean() * 100:.1f}%)")
    print(f"  Mean Indirect: {df_strategy['jobs_indirect'].mean():.1f} ({df_strategy['jobs_indirect'].mean() / df_strategy['total_jobs'].mean() * 100:.1f}%)")
    print()
    ...
    for country, avg_jobs in country_avg.items():
        ...
        direct_avg = df_strategy[df_strategy['main_country'] == country]['jobs_direct'].mean()
        indirect_avg = df_strategy[df_strategy['main_country'] == country]['jobs_indirect'].mean()
        print(f"  {country}: {avg_jobs:.1f} jobs/$M (Direct: {direct_avg:.1f}, Indirect: {indirect_avg:.1f}, n={count})")
    print()
...
print("  - Jobs_Total_by_Country: Total jobs by country and strategy")
print("  - Jobs_Direct_by_Country: Direct jobs in sectors receiving investment")
print("  - Jobs_Indirect_by_Country: Indirect jobs in supply chain sectors (spillovers)")
...
print("  - Direct_Jobs_by_Sector: Sectoral distribution of direct jobs")
print("  - Indirect_Jobs_by_Sector: Sectoral distribution of indirect jobs (supply chain)")
print()
print("="*80)
