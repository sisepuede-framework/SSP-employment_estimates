...
print(f"\n✓ Converted investment to final demand")
...
print(f"✓ Leontief inverse loaded: {IO_model.L_BASE.shape}")
...
print(f"✓ Initial final demand (dy): ${dy_inv_exog.sum():,.0f}")

# Calculate total output (direct + indirect via Leontief)
dq_total = IO_model.calc_dq_exog(dy_inv_exog)

print(f"✓ Total output change (dq): ${dq_total.sum():,.0f}")
print(f"✓ Output multiplier: {dq_total.sum() / dy_inv_exog.sum():.3f}x")
...
print(f"\n✓ Employment Calculations Complete:")
...
print(f"\n✓ Direct/Total Ratio: {dempl_direct.sum() / dempl_total.sum():.3f}")
print(f"✓ Indirect/Total Ratio: {dempl_indirect.sum() / dempl_total.sum():.3f}")
...
print(f"✓ Formatted results by region and sector")

# Display top 10 regions by total employment
print(f"\n✓ Top 10 Regions by Total Employment:")
...
print(f"\n✓ Top 10 Sectors by Total Employment:")
...
if 5 <= total_multiplier <= 50:
    print(f"  Employment multiplier ({total_multiplier:.2f} jobs/$1M): ✓ Within expected range (5-50)")
else:
...
if output_mult > 1.0:
    print(f"  Output multiplier ({output_mult:.3f}): ✓ Greater than 1.0")
else:
...
print(f"\n✓ Methodology validated for batch processing!")
print("\n" + "="*80 + "\n")
