...
Modified version of InputOutput.py for SSP analysis (8 aggregated regions)

CHANGES FROM ORIGINAL:
- Lines 197, 232, 259: .mat file paths point to SSP\ subfolder
- This ensures L_BASE, Y_BASE, G_BASE are loaded from GLORIA_db\v57\2019\SSP\
- Matches the location of aggregated .pkl files (labor_data, IND_sparse, etc.)

Original author: wb582890
Modified for SSP: 2026-03-22
"""
...
class IO:
    ...
    SSP VERSION: Loads .mat files from SSP\ subfolder for 8-region aggregation.
    """
    ...
    def invert_B_base(self):
        ...
        # SSP MODIFICATION: Save to SSP\ subfolder
        scipy.io.savemat(
            self.IO_PATH + f"GLORIA_db\\{self.GLORIAv}\\{str(self.year)}\\SSP\\GLORIA_G_Base_{str(self.year)}.mat",
            {"G_base": self.L_BASE})
    ...
    def invert_A_base(self):
        ...
        # SSP MODIFICATION: Save to SSP\ subfolder
        scipy.io.savemat(
            self.IO_PATH + f"GLORIA_db\\{self.GLORIAv}\\{str(self.year)}\\SSP\\GLORIA_L_Base_{str(self.year)}.mat",
            {"L_base": self.L_BASE})
    ...
    def load_Y_base(self):
        if 'Y_BASE' in dir(self):
            ...
            # SSP MODIFICATION: Save to SSP\ subfolder
            scipy.io.savemat(
                self.IO_PATH + f"GLORIA_db\\{self.GLORIAv}\\{str(self.year)}\\SSP\\GLORIA_Y_Base_{str(self.year)}.mat",
