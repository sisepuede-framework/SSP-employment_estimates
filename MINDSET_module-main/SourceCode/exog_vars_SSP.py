...
Modified for SSP analysis (8 aggregated regions)
Uses Variable_list_MINDSET_SSP.xlsx and aggregated data from SSP\ folder
...
class exog_vars:
    def __init__(self, IO_PATH = os.getcwd() + "\\"):
        ...
        # USE SSP-SPECIFIC VARIABLE LIST
        var_path = self.IO_PATH + "GLORIA_template\\Variables\\Variable_list_MINDSET_SSP.xlsx"
        ...
        # COU_ID and SEC_ID come from R and P sheets (already set to 8 regions by collapse_MRIO_SSP.py)
        # No filtering needed - aggregated data in SSP\ folder already has only 8 regions
        self.COU_ID = self.R['Region_acronyms'].tolist()

        # Get region names - check for different possible column names
        if 'Region_names' in self.R.columns:
            self.COU_NAME = self.R['Region_names'].tolist()
        elif 'Region' in self.R.columns:
            self.COU_NAME = self.R['Region'].tolist()
        else:
            # Use second column as fallback (usually region names)
            self.COU_NAME = self.R.iloc[:, 1].tolist()

        self.SEC_ID = self.P['Lfd_Nr'].tolist()

        # Get sector names - check for different possible column names
        if 'Product' in self.P.columns:
            self.SEC_NAME = self.P['Product'].tolist()
        elif 'Sector' in self.P.columns:
            self.SEC_NAME = self.P['Sector'].tolist()
        else:
            # Use second column as fallback
            self.SEC_NAME = self.P.iloc[:, 1].tolist()
        ...
        # Add default for NPISH_BASE if not loaded
        if not hasattr(self, 'NPISH_BASE'):
            self.NPISH_BASE = pd.DataFrame(columns=['REG_exp', 'TRAD_COMM', 'NPISH'])

        # Add default for INV_BASE if not loaded
        if not hasattr(self, 'INV_BASE'):
            self.INV_BASE = pd.DataFrame(columns=['REG_exp', 'TRAD_COMM', 'REG_imp', 'VDFA'])
