...
string = "^FullResults*.*.xlsx$"
out_file = "all_long_form"
...
results_path = "{}\\GLORIA_results\\".format(os.getcwd())

files = [f for f in os.listdir(results_path) if re.search(string, f)]
...
with alive_bar(monitor=None, stats=None, title="Writing CSV...") as bar:
    df.to_csv(results_path + "\\{}.csv".format(out_file)) #type:ignore
